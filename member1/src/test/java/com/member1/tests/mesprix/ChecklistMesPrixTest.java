...
import com.member1.pages.backend.mesprix.LoginPage;
import com.member1.pages.backend.mesprix.OrdersPage;
import com.member1.pages.backend.mesprix.AdminMenuPage;
import com.member1.pages.backend.mesprix.DashboardPage;
import com.member1.pages.backend.mesprix.EditOrderPage;
...
import com.member1.pages.customer.product.MembershipPage;
import com.member1.pages.customer.product.ContactUsPage;
import com.member1.pages.customer.product.FaqEtServiceClientPage;
import com.member1.pages.customer.product.GeneralConditionPage;
import com.member1.pages.customer.product.MerciPage;
import com.member1.pages.customer.product.HomePage;
import com.member1.pages.customer.product.PaymentPage;
import com.member1.pages.customer.product.PersonalDataPage;
...
public class ChecklistMesPrixTest extends BaseDriverManager {
	...
	public void TC_PSP_01_VerifySubscribePageNavigation() {
		...
		try {
			...
			homePage.navigateURL(PRODUCT_URL);
			...
			if (headerFooter.isBecomeAPrivilegeMemberLinkDisplayed()) {
				Report.pass("'Become A Privilege Member' link on Footer.", getScreenshot(getDriver()));
			} else {
				Report.fail("'Become A Privilege Member' link on Footer.", getScreenshot(getDriver()));
			}
...
//			}	
			...
			String mespricBackendURL = PRODUCT_URL + "wp-admin/";
			
			Report.info("Navigate to '"+mespricBackendURL+"' url on browser.");
			loginPage.navigateToMesPrixBackEnd(mespricBackendURL);		
						
			if (loginPage.isMesPrixLogoDisplayed()) {
				Report.pass("Mes prix logo on Login page.", getScreenshot(getDriver()));
			} else {
				Report.fail("Mes prix logo on Login page.", getScreenshot(getDriver()));
			}
			
			String email = "nikunj@member1.co";
			String password = "Q^Ct5$1qNV2ytk4P6%e7l(V!";
	...
	public void test_PSP_13_VerifyProductOrderRefundOnBackend() {
		try {
			...
			String mesprixBackendURL = PRODUCT_URL + "wp-admin/";
			
			Report.info("Navigate to '"+mesprixBackendURL+"' url on browser.");
			loginPage.navigateToMesPrixBackEnd(mesprixBackendURL);		
...
//	}
	...
	public void TC_PSP_15_VerifyCompanyNameOnLegalTerms() {
		...
		try {
			...
			// Home page > Header
			String email = Enums.MesPrixDetails.EMAIL.getValue();			
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on header section on Home page.", getScreenshot(getDriver()));
			}
		
			String contactNumber = Enums.MesPrixDetails.CONTACT_NUMBER.getValue();
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on header section on Home page.", getScreenshot(getDriver()));
			}
			
			// Home page > Content
			HomePage homePage = new HomePage(getDriver());
			
			String monthPrice = Enums.MesPrixDetails.MONTH_PRICE.getValue();
			...
			if (actualMonthPriceCount==expectedMonthPriceCount) {
				...
				Report.fail("Expected month price '"+monthPrice+"' appear <b>'"+expectedMonthPriceCount+"'</b> times but found <b>'"+actualMonthPriceCount+"'</b> times on Home page.", getScreenshot(getDriver()));
			}
			
			// Home page > Footer
			String companyNameWithAddress = Enums.MesPrixDetails.COMPANY_NAME.getValue() + " " + Enums.MesPrixDetails.COMPANY_ADDRESS.getValue();
			int actualCompanyNameWithAddressCount = headerFooter.getCompanyFullAddressCountOnFooter(Enums.MesPrixDetails.COMPANY_NAME.getValue(),Enums.MesPrixDetails.COMPANY_ADDRESS.getValue());
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on Home page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on footer section on Home page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on footer section on Home page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on footer section on Home page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on header section on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on header section on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualMonthPriceCount==expectedMonthPriceCount) {
				...
				Report.fail("Expected month euro price '"+monthEuroPrice+"' appear <b>'"+expectedMonthPriceCount+"'</b> times but found <b>'"+actualMonthPriceCount+"'</b> times on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on footer section on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on footer section on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on footer section on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on footer section on Membership page.", getScreenshot(getDriver()));
			}
			...
			actualCompanyNameWithAddressCount = membershipPage.getCompanyAddressCountOnModal(Enums.MesPrixDetails.COMPANY_ADDRESS.getValue());
			expectedCompanyNameWithAddressCount = 1;
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name and address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on modal on Membership page.", getScreenshot(getDriver()));
			}
			
			actualCompanyNameWithAddressCount = membershipPage.getCompanyNameWithAddressCountOnModal(Enums.MesPrixDetails.COMPANY_NAME.getValue(),Enums.MesPrixDetails.COMPANY_ADDRESS.getValue());
			expectedCompanyNameWithAddressCount = 1;
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on modal on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on modal on Membership page.", getScreenshot(getDriver()));
			}	
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on modal on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected Price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on modal on Membership page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on header section on Contact us page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on header section on Contact us page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on Contact Us page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on Contact Us page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected price '"+price+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on Contact Us page.", getScreenshot(getDriver()));
			}
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on footer section on Contact Us page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on footer section on Contact Us page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on footer section on Contact Us page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on footer section on Contact Us page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on header section on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on header section on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			
			// Privacy policy: donnees-personnelles page > content
			
			String companyName = Enums.MesPrixDetails.COMPANY_NAME.getValue();
			int actualCompanyNameCount = personalDataPage.getCompanyNameCount(Enums.MesPrixDetails.COMPANY_NAME.getValue());
			...
			if (actualCompanyNameCount==expectedCompanyNameCount) {
				...
				Report.fail("Expected company name '"+companyName+"' appear <b>'"+expectedCompanyNameCount+"'</b> times but found <b>'"+actualCompanyNameCount+"'</b> times on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			
			String companyAddress = Enums.MesPrixDetails.COMPANY_ADDRESS.getValue();
			...
			if (actualCompanyAddressCount==expectedCompanyAddressCount) {
				...
				Report.fail("Expected company address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyAddressCount+"'</b> times but found <b>'"+actualCompanyAddressCount+"'</b> times on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			
			actualCompanyNameWithAddressCount = personalDataPage.getCompanyNameWithAddressCount(Enums.MesPrixDetails.COMPANY_NAME.getValue(),Enums.MesPrixDetails.COMPANY_ADDRESS.getValue());
			expectedCompanyNameWithAddressCount = 1;
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on Donnes Personnelles page.", getScreenshot(getDriver()));
			}	
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"' </b> times but found <b>'"+actualEmailCount+"'</b> times on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on footer section on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on footer section on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on footer section on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on footer section on Donnes Personnelles page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on header section on FAQ Et Service Client page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on header section on FAQ Et Service Client page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on FAQ Et Service Client page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on FAQ Et Service Client page.", getScreenshot(getDriver()));
			}	
			...
			if (actualMonthPriceCount==expectedMonthPriceCount) {
				...
				Report.fail("Expected Price '"+monthEuroPrice+"' appear <b>'"+expectedMonthPriceCount+"'</b> times but found <b>'"+actualMonthPriceCount+"'</b> times on FAQ Et Service Client page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected Price '"+faqSectionPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on FAQ Et Service Client page.", getScreenshot(getDriver()));
			}
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on header section on GENERAL CONDITIONS OF SALE page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on header section on GENERAL CONDITIONS OF SALE page.", getScreenshot(getDriver()));
			}
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name and address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on GENERAL CONDITIONS OF SALE page.", getScreenshot(getDriver()));
			}
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on GENERAL CONDITIONS OF SALE page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on GENERAL CONDITIONS OF SALE page.", getScreenshot(getDriver()));
			}	
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on GENERAL CONDITIONS OF SALE page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected Price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on GENERAL CONDITIONS OF SALE page.", getScreenshot(getDriver()));
			}
			...
			Report.info("Navigate to product details page.");
			mesPrixHomePage.navigateURL(productUrl);

			monthEuroPrice = "29,90 EUR/mois";
			...
			int actualCompanyNameAndAddressCount = paymentPage.getCompanyNameAndAddressCount(Enums.MesPrixDetails.COMPANY_NAME.getValue(),Enums.MesPrixDetails.COMPANY_ADDRESS.getValue());
			...
			if (actualCompanyNameAndAddressCount==expectedCompanyNameAndAddressCount) {
				...
				Report.fail("Expected company name and address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameAndAddressCount+"'</b> times but found <b>'"+actualCompanyNameAndAddressCount+"'</b> times on Payment page.", getScreenshot(getDriver()));
			}
			
			actualCompanyNameWithAddressCount = paymentPage.getCompanyNameWithAddressCount(Enums.MesPrixDetails.COMPANY_NAME.getValue(),Enums.MesPrixDetails.COMPANY_ADDRESS.getValue());
			expectedCompanyNameWithAddressCount = 1;
			...
			if (actualCompanyNameWithAddressCount==expectedCompanyNameWithAddressCount) {
				...
				Report.fail("Expected company name with address '"+companyNameWithAddress+"' appear <b>'"+expectedCompanyNameWithAddressCount+"'</b> times but found <b>'"+actualCompanyNameWithAddressCount+"'</b> times on Payment page.", getScreenshot(getDriver()));
			}
			...
			if (actualContactNumberCount==expectedContactNumberCount) {
				...
				Report.fail("Expected contact number '"+contactNumber+"' appear <b>'"+expectedContactNumberCount+"'</b> times but found <b>'"+actualContactNumberCount+"'</b> times on Payment page.", getScreenshot(getDriver()));
			}	
			...
			if (actualEmailCount==expectedEmailCount) {
				...
				Report.fail("Expected email '"+email+"' appear <b>'"+expectedEmailCount+"'</b> times but found <b>'"+actualEmailCount+"'</b> times on Payment page.", getScreenshot(getDriver()));
			}
			...
			if (actualPriceCount==expectedPriceCount) {
				...
				Report.fail("Expected Price '"+bothPrice+"' appear <b>'"+expectedPriceCount+"'</b> times but found <b>'"+actualPriceCount+"'</b> times on Payment page.", getScreenshot(getDriver()));
			}
