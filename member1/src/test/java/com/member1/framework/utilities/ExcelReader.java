...
public class ExcelReader {
	...
	/**
	 * Performs "UPDATE SET <ColumnName> = <Value> Where <whereClause>". Updates
	 * data that meet the condition(s) given in where clause, to an excel sheet.
	 *
	 * @param excelName   - Excel file name
	 * @param sheetName   - Sheet name in an excel file
	...
	public static void updateColumn(String excelName, String sheetName, String columnName, String columnValue,
		...
		try {
			Connection connection = fillo
					.getConnection(Path.getProjectDir() + File.separator + testDataPath + File.separator + excelName);
			String query = "Update " + sheetName + " Set " + columnName + "='" + columnValue + "' where " + whereClause;
	...
	/**
	 * Performs "SELECT * FROM <SheetName>". fetch data that meet the condition(s),
	 * from an excel sheet.
	 *
	 * @param excelName - Excel file name
	 * @param sheetName - Sheet name in an excel file
	 * @return List<HashMap<String, String>> Array of list with key value pair of
	 *         each row.
	 */
